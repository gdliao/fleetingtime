...
function initMyBooking(){
	...
	$.ajax({
		...
		type : "POST",
		async:false,
		url : window.contextPath+"/booking/queryMyBookingInfo.action",
	...
	$.messager.confirm('删除记录', '确认删除该预约记录？', function(r){
		if(r){
			$.ajax({
				...
				async:false,
				url : window.contextPath+"/booking/deleteRecord.action",
				success : function(data) {
					if(data.result=='success'){
						alert("删除成功！");
						initMyBooking();
	...
	$.messager.confirm('修改提交', '<br>请确认预约内容：<br><br>开始时间：'+startTime+"<br>结束时间："+endTime+"<br>会议室："+$("#roomId").find("option:selected").text()+"<br>主题："+title+ (equipNames==""?"":("<br>设备："+equipNames)), function(r){
		if(r){
			$.ajax({
				...
				async:false,
				url : window.contextPath+"/booking/editMyBooking.action",
				success : function(data) {
					if(data.result=='success'){
						alert("修改成功！");
						initMyBooking();
